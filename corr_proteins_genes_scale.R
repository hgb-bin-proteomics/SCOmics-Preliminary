...
data_scaled <- data[,3:7] |> mutate_all(~(scale(.) %>% as.vector)) 
data_m <- data.matrix(data_scaled)
normalized <- data_m
...
# median normalization
data_scaled_ngs <- data_ngs[,2:6] |> mutate_all(~(scale(.) %>% as.vector)) 
data_m_ngs <- data.matrix(data_ngs_scaled)
normalized_ngs <- data_m_ngs
